...
IF tz1(w) > 0 OR tz2(w) > 0 THEN
    ...
    IF y1a < -3000 THEN y1a = -3000
    IF y1b > 3000 THEN y1b = 3000
    IF y2a < -3000 THEN y2a = -3000
    IF y2b > 3000 THEN y2b = 3000
    ...
    IF x2 >= x1 THEN
        ...
        FOR x = mix TO max
            ...
            LINE (50 + x, 50 + yb)-(50 + x, 120), 1
            LINE (50 + x, 50 + ya)-(50 + x, 50 + yb), wc(w)
            GOSUB k
        NEXT
        IF x1 >= -100 AND x1 <= 100 THEN LINE (50 + x1, 50 + y1a)-(50 + x1, 50 + y1b), wc(w) AND 7
        IF x2 >= -100 AND x2 <= 100 THEN LINE (50 + x2, 50 + y2a)-(50 + x2, 50 + y2b), wc(w) AND 7
    END IF
END IF
RETURN
k:
