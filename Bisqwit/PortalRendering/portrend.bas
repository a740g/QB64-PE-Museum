...
SUB RenderSector (sectorno, sx1, sx2)
    ...
    FOR s = 1 TO sectors(sectorno).npoints
        ...
        x1 = W \ 2 + CLNG(-tx1 * xscale1)
        x2 = W \ 2 + CLNG(-tx2 * xscale2)
        ...
        y1a = H \ 2 + CLNG(-yceil * yscale1): y1b = H \ 2 + CLNG(-yfloor * yscale1)
        y2a = H \ 2 + CLNG(-yceil * yscale2): y2b = H \ 2 + CLNG(-yfloor * yscale2)
        ...
        IF neighbor >= 0 THEN
            ...
            ny1a = H \ 2 + CLNG(-nyceil * yscale1): ny1b = H \ 2 + CLNG(-nyfloor * yscale1)
            ny2a = H \ 2 + CLNG(-nyceil * yscale2): ny2b = H \ 2 + CLNG(-nyfloor * yscale2)
        END IF
        ' Just the wall.
...
FUNCTION FNib (a0, a1, b0, b1)
    FNib = (a0 < b1) AND (a1 > b0)
END FUNCTION
...
FUNCTION FNintersectBox% (x0, y0, x1, y1, x2, y2, x3, y3)
    FNintersectBox = FNib(FNmin(x0, x1), FNmax(x0, x1), FNmin(x2, x3), FNmax(x2, x3)) AND FNib(FNmin(y0, y1), FNmax(y0, y1), FNmin(y2, y3), FNmax(y2, y3))
END FUNCTION

FUNCTION FNpointSide% (px, py, x0, y0, x1, y1)
    FNpointSide = SGN((x1 - x0) * (py - y0) - (y1 - y0) * (px - x0))
END FUNCTION

DEFINT A-Z
FUNCTION FNclamp (value, min, max)
    SELECT CASE value
