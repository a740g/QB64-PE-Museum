...
' the new (6/25/2022 forms designer should only extract and insert between start and end of GUI Controls line

Xmax = 1280: Ymax = 700: GuiTitle$ = "GUI Form Designer": FontFile$ = "ARLRDBD.TTF"
OpenWindow Xmax, Ymax, GuiTitle$, FontFile$ ' need to do this before drawing anything from NewControls"

' GUI Controls
'                     Dim and set Globals for GUI app
Dim Shared As Long Btn1, Tb1, Lbl1, Pic1, Lst1, lblPractice
Btn1 = NewControl(1, 9, 80, 652, 40, 30, 0, 0, "Btn 1 Here")
Tb1 = NewControl(2, 10, 140, 282, 39, 30, 0, 0, "Text Box 1")
Lbl1 = NewControl(4, 310, 141, 349, 37, 30, 0, 0, "Label 1")
Pic1 = NewControl(5, 10, 200, 280, 280, 16, 0, 0, "Picture 1")
Lst1 = NewControl(3, 310, 200, 350, 280, 20, 0, 0, "List Box 1")
lblPractice = NewControl(4, 10, 500, 650, 40, 38, 0, 191, "This is a practice label.")
' End GUI Controls
    ...
    Select Case ConType
        ...
        Case 2
            ...
            Else ' convert to RGB
                con(NControls).FC = C3(FC): con(NControls).BC = C3(BC)
            End If
            If FontH > H - 8 Then con(NControls).FontH = H - 8 Else con(NControls).FontH = FontH
        ...
        Case 3
            ...
            'con(NControls).N3 = Int((W - 16) / 8) ' page width - .5 charcter margin on each side 1 char scroll click bar
...
Sub MainRouter
    Dim As Long kh, mx, my, mb1, mb2, i, shift, xx, yy
    Do
        ...
        If mb1 Then ' find which control
            ...
            ElseIf con(ActiveControl).ConType = 2 Then ' move cursor to click point
                If mx > con(ActiveControl).X + 4 And mx < con(ActiveControl).X + con(ActiveControl).W Then
                    If my >= con(ActiveControl).Y And my <= con(ActiveControl).Y + con(ActiveControl).H Then
                        con(ActiveControl).N2 = Int((mx - (con(ActiveControl).X + 4)) / con(ActiveControl).N3) + 1
                        If (con(ActiveControl).N1 - 1) * con(ActiveControl).N4 + con(ActiveControl).N2 > con(ActiveControl).N5 Then ' ky end
            ...
            ElseIf con(ActiveControl).ConType = 3 Then
                ...
                ElseIf my >= con(ActiveControl).Y + con(ActiveControl).FontH And my < con(ActiveControl).Y + con(ActiveControl).H - con(ActiveControl).FontH Then
                    ...
                    If (con(ActiveControl).N1 - 1) * con(ActiveControl).N4 + con(ActiveControl).N2 > con(ActiveControl).N5 Then LstKeyEvent ActiveControl, 20224 ' end
        ...
        If mb2 Then ' use right clicking to select
            If con(ActiveControl).ConType = 3 Then ' this does not make the lst active but if is can select the highlited
                ...
                ' OK mBox "Debug", "Active Control:" + con(ac).Text  = ac = 5 and correct Text is showing
...
Sub drwLbl (i As Long)
    ...
    If con(i).FontH >= 20 Then
        Color Black
        _PrintString (con(i).X + (con(i).W - _PrintWidth(con(i).Text)) / 2 + 1, con(i).Y + 1), con(i).Text
    End If
    Color con(i).FC
...
Sub drwPic (i As Long, active As Long)
    If con(i).Text <> "" Then ' title to display
        ...
        _Dest con(i).N1
        _PrintMode _KeepBackground
        Line (0, con(i).H - con(i).FontH - 2)-Step(con(i).W - 1, con(i).FontH + 2), &HFF008800, BF ' pic title
        _Font fontHandle&(con(i).FontH)
