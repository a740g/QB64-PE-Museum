'
'  Copyright 2023 Arnold Kramer, T&T SOFTWARE
'
...
_TITLE "Helicopter Rescue - Replay"
...
_SNDVOL Gun&, Volumex / 100
_SNDVOL HitEnemy&, Volumex / 100
_SNDVOL HitPlay&, Volumex / 100
_SNDVOL Bonus1&, Volumex / 100
_SNDVOL Bonus2&, Volumex / 100
_SNDVOL Bonus3&, Volumex / 100
_SNDVOL Bonus4&, Volumex / 100
_SNDVOL Bonus5&, Volumex / 100
_SNDVOL Bonus6&, Volumex / 100
_SNDVOL Fail&, Volumex / 100
_SNDVOL Dingdong&, Volumex / 100
_SNDVOL Rescue&, Volumex / 100
_SNDVOL Blades&, Volumex / 100
_SNDVOL Damage&, Volumex / 100
_SNDVOL Garage&, Volumex / 100
_SNDVOL ThemeA&, Volumex / 100
_SNDVOL ThemeB&, Volumex / 100
_SNDVOL ThemeC&, Volumex / 100
_SNDVOL Click&, Volumex / 100
...
FOR y = 0 TO 31
    ...
    FOR x = 0 TO LEN(line$) - 1
        ...
        IF MID$(line$, x + 1, 1) = "2" THEN PSET (x, y), 131
...
noe& = 0
'****************************************************************** CACHE
PALETTE 255, 65536 * 0 + 256 * 63 + 0 'hoofdkleur
waarde$ = "CACHING REPLAY.MCR...": dx = 400: dy = 200: GOSUB PlaatsTekstCentered
...
IF i$ = "+" AND muxer! < 4 THEN yesss = 1: _SNDPLAY Click&: muxer! = muxer! * 2: ShowSpeed = 1: OverallFramerate = OverallFramerate * 2: yesssteller& = 0
IF i$ = "-" AND muxer! > 1 / 4 THEN yesss = 1: _SNDPLAY Click&: muxer! = muxer! / 2: ShowSpeed = 1: OverallFramerate = OverallFramerate / 2: yesssteller& = 0
...
SELECT CASE Controller$
    CASE "MOUSE"
        ...
        IF PlayDemo = 1 THEN
            ...
            IF HMuis = 9999 THEN
                DonderOp = 1
                HMuis = 400: Vmuis = 225: BMuis = 0: TR = 0
            ELSE
...
'
GOSUB HorizonPaint '
'Locate 20, 1: Print StappenTeller& '
        ...
        IF yk#(akn) < 10 THEN
            FOR ObjectNummer = 1 TO AantalObjecten
                ...
                IF d# < ObjectHitBall AND yk#(akn) < HithoogteOmax AND ObjectYesNo(ObjectNummer) = 1 THEN
                    ...
                    '
                    ShowObjects = 1
                    IF ObjectsHit = 1 * (AantalObjecten \ 4) AND Bonus1 = 0 THEN score& = score& + 200: Bonus1 = 1: _SNDPLAY Bonus1&
                    IF ObjectsHit = 2 * (AantalObjecten \ 4) AND Bonus2 = 0 THEN score& = score& + 200: Bonus2 = 1: _SNDPLAY Bonus2&
                    IF ObjectsHit = 3 * (AantalObjecten \ 4) AND Bonus3 = 0 THEN score& = score& + 200: Bonus3 = 1: _SNDPLAY Bonus3&
                    IF ObjectsHit = 4 * (AantalObjecten \ 4) AND Bonus4 = 0 THEN

                        Tempsec& = Tempsec& + 1 '  correctie

                        score& = score& + 2000: Bonus4 = 1: _SNDPLAY Bonus4&: GOSUB NewObjects
...
FOR AEN = 1 TO NumberOfEnemiesActive
    ...
    yU(AEN) = 450 'voorlopige y-waarde Elevation  '450 was 600  !!!
    FOR V = 0 TO l(AEN) - 1
        jn0 = 0: jn1 = 0: jn2 = 0: jn3 = 0: jn4 = 0
...
IF radar$ = "ON" THEN
    FOR altpsn = 1 TO waypoints
        IF rescue(altpsn) = 0 THEN
            ...
            Alpha# = 2 * pi# - Koers#
            Beta# = _ATAN2((zwp#(altpsn) - zc0#), (xwp#(altpsn) - xc0#))
            RescueRadarhoek# = .5 * pi# - (Alpha# + Beta#)
...
IF UCASE$(i$) = "!" THEN GOSUB swapDevMode '
'   *************************************************************
...
'
IF TypeObjecten MOD 2 = 0 AND INT(xc0#) = 1313 AND INT(zc0#) = 1313 AND EasterEgg1 = 0 THEN EasterEgg1 = 1: score& = score& + 5000: _SNDPLAY Bonus5& '  1313 easteregg
...
'Locate 1, 1: Print Hmuis; VMuis
'
IF Exit$ = "" THEN Exit$ = " " 'was  "- TOTAL LOSS -"
waarde$ = Exit$: dx = 400: dy = 210: GOSUB PlaatsTekstCentered '     210 was 170
waarde$ = "                                V1.6.19": dx = 400: dy = 305: GOSUB PlaatsTekstCentered
...
WHILE INKEY$ = ""
    ...
    Mouse 3
    IF BMuis = 1 GOTO ExitMenu
    totalteller = totalteller + 1
    IF totalteller > 4 * OverallFramerate THEN _DISPLAY: GOTO ExitMenu
WEND
GOTO ExitMenu
'**************************************************************************
'
...
IF devmodePrint = 1 THEN
    ...
    dx = 8: dy = 45: waarde$ = LTRIM$(STR$(framespark)) + " FPS": GOSUB PlaatsTekstLinksUitlijn
END IF
    ...
    FOR AEN = 1 TO NumberOfEnemiesActive
        ...
        IF Distance#(AEN) <= 4000 AND Distance#(AEN) > 2000 THEN '                        Voorwaarden
            IF Radarhoek#(AEN) > 1.75 * pi# OR Radarhoek#(AEN) < .25 * pi# THEN '        grote radar
                ...
                SELECT CASE RadarDotType(AEN) ' 0=friend 1=enemy ************************
                    CASE 0
                        IF Wreck(AEN) = 0 THEN CIRCLE (xRadar(AEN), yRadar(AEN)), 2, 15 '     was 164      15
                    CASE 1
        ...
        IF Distance#(AEN) <= 2000 THEN '                 Voorwaarden kleine radar
            ...
            SELECT CASE RadarDotType(AEN)
                CASE 0
                    IF Wreck(AEN) = 0 THEN CIRCLE (xRadar(AEN), yRadar(AEN)), 2, 15 '
                CASE 1
                    IF Wreck(AEN) = 0 THEN LINE (xRadar(AEN) - 1, yRadar(AEN) - 1)-(xRadar(AEN) + 1, yRadar(AEN) + 1), roodflits(AEN), BF
            END SELECT
            IF Wreck(AEN) = 0 AND firstTimeWarnNOT(AEN) = 0 THEN PLAY "v" + Pct$ + "MBl32o4a": firstTimeWarnNOT(AEN) = 1: roodflits(AEN) = 249
...
DATA ....11111111112.......11111111112......
DATA ....11111111112.......11111111112......
DATA ....11111111112......211111111112......
DATA ....11111111112....2211111111112.......
DATA ....111111111122221111111111112........
...
DATA ....11111111112......1111111111112.....
DATA ....11111111112.......1111111111112....
DATA ...2111111111112.......11111111111112..
DATA 22111111111111111112....111111111111112
...
DATA 443333333333333333333333333333333333333333333322
DATA 443333333333355535555335533555335555333333333322
DATA 443333333333555335555355553533335533333333333322
DATA 443333333333333535333353353533335333333333333322
DATA 443333333333555335333353353355535555333333333322
DATA 443333333333333333333333333333333333333333333322
DATA 443333333333333333333333333333333333333333333322
