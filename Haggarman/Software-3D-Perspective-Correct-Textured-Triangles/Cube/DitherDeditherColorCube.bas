Option _Explicit
_Title "Dither De-Dither Color Cube"
' 2023 Haggarman
' Draw a few transparent cubes in 3D. Press S to stop the cubes in a nice spot. Press D multiple times as you compare the results.
'
' De-dither is such a simple idea to recover lost bits of color info as the video signal is streamed out.
' Expand a pixel 0RRRRRGGGGGBBBBB into RRRRR000 GGGGG000 BBBBB000
' For each of the 8 pixel neighbor's RGB values, add 1 if the neighbor is greater than the center color.
'  else subtract 1 if the neighbor is less than the center color. If equal, leave alone (add 0).
' The N64 needed to use this because of terrible rainbow shimmering on NTSC.
'  Yes 3 entire horizontal lines need to be buffered internally to perform dedither while streaming the video output signal.
'   But in this simulation we just use a full size buffer named FILTER_IMAGE and iterate over every pixel. Literal edge cases are skipped.
' Later 3dfx cards after Voodoo2 would use a 2 by 2 box filter (simple averaging) as they stuck with 15-bit when the competition went to 24-bit rendering.
'
