...
Dim Shared Affine_Only As Integer
...
Do
    ...
    Print "Press A for Affine/Perspective: ";
    If Affine_Only = 1 Then
        Print "Affine, no 1/Z adjustment"
    Else
    ...
    If KeyNow <> "" Then
        ...
        ElseIf KeyNow = "A" Then
            Affine_Only = Affine_Only + 1
            If Affine_Only > 1 Then Affine_Only = 0
        ...
        If delta_x >= (1 / 2048) Then
            ...
            While col < draw_max_x
                ...
                If Screen_Z_Buffer(zbuf_index) = 0.0 Or tex_z < Screen_Z_Buffer(zbuf_index) Then
                    ...
                    If Affine_Only Then
                        pixel_combine = RGB_Lit&(RGB_Modulate&(ReadTexel&(tex_u, tex_v), _RGB32(tex_r, tex_g, tex_b)))
                        pixel_alpha = tex_a
                    Else
                        ' Persepective Correct
                        ' !!! notice the only difference is multiplying by tex_z (interpolated depth)
                        pixel_combine = RGB_Lit&(RGB_Modulate&(ReadTexel&(tex_u * tex_z, tex_v * tex_z), _RGB32(tex_r * tex_z, tex_g * tex_z, tex_b * tex_z)))
